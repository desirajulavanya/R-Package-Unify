test_that("audit_panel identifies balanced panels", {
  ...
  expect_s3_class(audit, "panelbuild_panel_audit")
...
test_that("flag_panel_issues adds duplicate flags without removing rows", {
  ...
  expect_true("panelbuild_row_id" %in% names(flagged))
  expect_true("panelbuild_id_time_n" %in% names(flagged))
  expect_true("panelbuild_duplicate_cell" %in% names(flagged))

  expect_equal(sum(flagged$panelbuild_duplicate_cell), 2)
  expect_equal(attr(flagged, "panelbuild_id"), "district")
  expect_equal(attr(flagged, "panelbuild_time"), "year")
})
...
test_that("complete_panel adds missing id-time cells with an audit trail", {
  ...
  expect_true("panelbuild_original_row" %in% names(completed))
  expect_true("panelbuild_completed_cell" %in% names(completed))
  expect_true("panelbuild_audit_action" %in% names(completed))

  expect_equal(sum(completed$panelbuild_completed_cell), 1)
  expect_equal(attr(completed, "panelbuild_completed_cells"), 1)

  added <- completed[completed$panelbuild_completed_cell, ]
