...
test_that("gap_summary reports missing periods by unit", {
  ...
  expect_s3_class(summary, "tbl_df")
  expect_true("unfiy_missing_periods" %in% names(summary))
  expect_equal(summary$unfiy_missing_periods[summary$district == "B"], 2)
  expect_equal(summary$unfiy_missing_periods[summary$district == "A"], 1)
  expect_equal(summary$unfiy_missing_periods[summary$district == "C"], 1)
})
...
test_that("duplicate_summary reports duplicate cells by unit", {
  ...
  expect_s3_class(summary, "tbl_df")
  expect_true("unfiy_duplicate_cells" %in% names(summary))
  expect_true("unfiy_duplicate_extra_rows" %in% names(summary))

  expect_equal(summary$unfiy_duplicate_cells[summary$district == "B"], 1)
  expect_equal(summary$unfiy_duplicate_extra_rows[summary$district == "B"], 1)

  expect_equal(summary$unfiy_duplicate_cells[summary$district == "C"], 1)
  expect_equal(summary$unfiy_duplicate_extra_rows[summary$district == "C"], 1)
})
